...
upsertSql' ent updateVal = pack $ concat
  [ "MERGE ", escapeDBName (entityDB ent), " AS [Target] "
  , "USING (SELECT ", intercalate "," (map (\x -> "? AS " ++ escapeDBName (fieldDB x)) (entityFields ent)),  ") AS [Source] "
  , "ON ", intercalate "," (concatMap (map (\x -> "[Target]." ++ escapeDBName (snd x) ++ " = [Source]." ++ escapeDBName (snd x)) . uniqueFields) (entityUniques ent))
  , " WHEN MATCHED THEN UPDATE SET ", T.unpack updateVal
  , " WHEN NOT MATCHED THEN INSERT "
  , " (", intercalate "," $ map (escapeDBName . fieldDB) $ entityFields ent, ") VALUES "
  , " (", intercalate "," $ map (\x -> "[Source]." ++ escapeDBName (fieldDB x)) $ entityFields ent, ")"
  ]
